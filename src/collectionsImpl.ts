...
export class SortedMap<K, V> {
    ...
    public has(key: K): boolean {
        return core.binarySearch(this._keys, key, core.identity, this._comparer) >= 0;
    }

    public get(key: K): V | undefined {
        const index = core.binarySearch(this._keys, key, core.identity, this._comparer);
    ...
    public delete(key: K): boolean {
        ...
        if (index >= 0) {
            this.writePreamble();
            core.orderedRemoveItemAt(this._keys, index);
            core.orderedRemoveItemAt(this._values, index);
            if (this._order) core.orderedRemoveItemAt(this._order, index);
            this.writePostScript();
            return true;
        }
        return false;
